...
With the v2 news flag enabled, the same 15-minute job covers twelve official RSS feeds (Fed, ECB, SEC, CFTC, BIS, BOJ, RBI, BoE, BoC, FDIC, NBS China, and the OFAC GovDelivery subscription topic) plus the official HKMA press-release JSON API. HKMA is a separate Hong Kong/Greater China banking and liquidity channel, NBS preserves original Chinese statistical releases and applies conservative deterministic Chinese rules, and OFAC adds deterministic sanctions events. The retired native OFAC RSS and HTML scraping are not used. Response schemas, dates, counts, duplicates, payload sizes, time zones, and allowlisted item hosts are validated before persistence. GDELT remains discovery-only, and an unavailable news channel reduces news coverage rather than fabricating a stable or crisis state.
...
python -m scripts.crisis_radar sync --source bybit_stablecoin_research
python -m scripts.crisis_radar sync --source oecd_labour_research
python -m scripts.crisis_radar sync --source imf_portwatch
python -m scripts.crisis_radar sync --source binance_market
...
FRED, BEA, and EIA keys enable their respective live synchronizations. ECB, Eurostat, World Bank, BIS, OECD, New York Fed GSCPI, IMF PortWatch, Bybit, Binance public market data, all configured official RSS feeds, and the HKMA press-release API use public endpoints without keys. World Bank, BIS, OECD, the disabled GSCPI collector, disabled version-pinned OECD non-US labour collector and disabled PortWatch chokepoint collector run in a separate daily job. The labour collector calculates five regional unemployment-momentum signals from 15 contiguous monthly observations, but assigns all five to one systemic labour cluster. PortWatch calculates seven-day transit shortfall against the preceding 365-day median for five key chokepoints; all five remain separate groups inside one systemic shipping/logistics cluster. FRED also collects disabled candidate-v19 reserve-balance quantity and commercial-paper funding-price series inside one dollar-liquidity cluster. The disabled Bybit/Binance USDC/USDT research collectors run hourly, validate executable bid/ask and store both venues in one dependency subchannel so they cannot double risk breadth. Every disabled collector has an isolated `research_candidate` sync run, so its outage remains visible to source health and canary without degrading required live channels. These research collectors require `CRISIS_RADAR_SCORING_V11_ENABLED=1` and remain disabled from live scoring until causal replay and promotion gates pass. The methodology and UI can be tested entirely offline with fixtures. The dashboard deliberately does not show a single “magic crash probability”: the overview communicates the current market stage and breadth of deterioration, while technical scores and source health stay behind the detailed-mode toggle.
