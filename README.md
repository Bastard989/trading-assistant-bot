...
FRED, BEA, and EIA keys enable their respective live synchronizations. ECB, Eurostat, World Bank, BIS, OECD, New York Fed GSCPI, Bybit, Binance public market data, all configured official RSS feeds, and the HKMA press-release API use public endpoints without keys. World Bank, BIS, OECD, the disabled GSCPI collector and the disabled version-pinned OECD non-US labour collector run in a separate daily job because their macro series update slowly. The labour collector calculates five regional unemployment-momentum signals from 15 contiguous monthly observations, but assigns all five to one systemic labour cluster. The disabled Bybit/Binance USDC/USDT research collectors run hourly, validate executable bid/ask and store both venues in one dependency subchannel so they cannot double risk breadth. Every disabled collector has an isolated `research_candidate` sync run, so its outage remains visible to source health and canary without degrading required live channels. These research collectors require `CRISIS_RADAR_SCORING_V11=1` and remain disabled from live scoring until causal replay and promotion gates pass. The methodology and UI can be tested entirely offline with fixtures. The dashboard deliberately does not show a single “magic crash probability”: the overview communicates the current market stage and breadth of deterioration, while technical scores and source health stay behind the detailed-mode toggle.
