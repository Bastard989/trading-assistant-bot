...
With the v2 news flag enabled, the same 15-minute job covers ten official RSS feeds (Fed, ECB, SEC, CFTC, BIS, BOJ, RBI, BoE, BoC, FDIC), the official HKMA press-release JSON API and the OFAC official e-mail subscription topic exposed through GovDelivery RSS. HKMA is a separate Hong Kong/Greater China banking and liquidity channel, not an RSS compatibility shim; OFAC adds deterministic sanctions events. The retired native OFAC RSS and HTML scraping are not used. Response schemas, dates, counts, duplicates and allowlisted item hosts are validated before persistence. GDELT remains discovery-only, and an unavailable news channel reduces news coverage rather than fabricating a stable or crisis state.
