...
безработицы Канады, Великобритании, Японии, Южной Кореи и Мексики,
`candidate-v18` — отключённый контур физического судоходного потока пяти
ключевых мировых проливов по IMF PortWatch, `candidate-v19` — отключённый
контур количества резервной ликвидности США и цены денежного фондирования.
Ни один из них не участвует в
live-стадии.
...
`candidate-v18` добавляет официальный публичный IMF PortWatch API для Суэцкого
и Панамского каналов, Баб-эль-Мандебского, Малаккского и Ормузского проливов.
Для каждого пролива среднее число проходов судов за последние семь полных дней
сравнивается с медианой только предыдущих 365 дней:
`max(0, (1 − mean(latest 7d) / median(previous 365d)) × 100)`. Текущее
семидневное окно исключено из baseline, поэтому показатель не нормализует сам
себя текущим сбоем.

Пять проливов остаются отдельными видимыми группами, но входят в один systemic
cluster `shipping_logistics`: одновременный сбой нескольких маршрутов расширяет
географическое подтверждение, однако не выдаётся за пять независимых механизмов.
Пороги различаются из-за разной обычной волатильности потоков и пока являются
кандидатными экономическими зонами. Исторический ArcGIS snapshot может
пересматриваться и не содержит точных release timestamps, поэтому сохраняется
только последнее преобразованное значение с консервативным временем первой
загрузки. Все пять рядов `enabled=false`; final-vintage история не является
causal replay и вероятность не создаётся.
Каждое новое сохранённое преобразованное значение, напротив, становится
point-in-time vintage с фактического времени его загрузки; именно такая будущая
история сможет участвовать в replay после накопления достаточного периода.

`candidate-v19` добавляет два официальных FRED-ряда внутри одной группы
долларовой ликвидности. `WRESBAL` показывает 90-дневное изменение резервных
остатков банков в ФРС — количество доступной резервной ликвидности. `CPFF`
показывает спред трёхмесячных финансовых коммерческих бумаг к ставке федеральных
фондов — цену краткосрочного необеспеченного фондирования. Это разные
подканалы, но один systemic cluster `dollar_liquidity_banks`, поэтому они
углубляют подтверждение и не удваивают системную ширину. Оба ряда имеют
проверяемые initial-release vintages; изолированный causal backfill сохранил
747 точек за 2022–2026 годы без revised/estimated rows и без создания live
snapshot. Пороги остаются кандидатными, оба индикатора `enabled=false`.
...
| Поток через Суэцкий канал, снижение | 20% | 35% | 50% | candidate-v18; latest 7d против previous 365d median |
| Поток через Панамский канал, снижение | 15% | 25% | 40% | candidate-v18; отдельная группа, общий shipping-кластер |
| Поток через Баб-эль-Мандеб, снижение | 20% | 35% | 50% | candidate-v18; физический логистический сигнал |
| Поток через Малаккский пролив, снижение | 8% | 12% | 18% | candidate-v18; более низкая обычная волатильность |
| Поток через Ормузский пролив, снижение | 25% | 50% | 75% | candidate-v18; не равен цене нефти или вероятности кризиса |
| Резервные остатки банков в ФРС, 90d | −5% | −10% | −20% | candidate-v19; количество ликвидности, требует ценового подтверждения |
| Commercial paper − fed funds, п.п. | 0,50 | 1,00 | 2,00 | candidate-v19; цена краткосрочного фондирования, не вероятность |
