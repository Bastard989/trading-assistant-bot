...
## Рост базы и хранение расчётных снимков

Один пятнадцатиминутный новостной цикл считается одним аналитическим событием:
все официальные ленты сначала загружаются последовательно, затем выполняется
ровно один общий пересчёт. Нельзя возвращать пересчёт после каждой отдельной
ленты — это умножает полный набор производных строк на число источников, не
добавляя новой информации.

Canary сохраняет размер основной БД, WAL, каталога backup и число полных
расчётных снимков. Скорость роста БД свыше 256 MiB/сутки и каталог копий свыше
50 GiB дают отдельное предупреждение.

Прореживание относится только к производным snapshot-таблицам. Сырые
observations, revisions, новости, события, alert/data-health transitions,
scorecards, сделки и журнал не удаляются. Полностью сохраняются последние двое
суток, затем остаётся один снимок на час в пределах 180 дней и один снимок на
сутки для более старой истории. Точные timestamps alert/data-health событий
защищены всегда.

Сначала только посмотреть план:

```bash
.venv/bin/python scripts/radar_snapshot_retention.py \
  --database /var/lib/trading-assistant/trading_bot.sqlite3
```

Применение невозможно без автоматически созданной и проверенной online-копии:

```bash
.venv/bin/python scripts/radar_snapshot_retention.py \
  --database /var/lib/trading-assistant/trading_bot.sqlite3 \
  --apply --backup-directory /var/lib/trading-assistant/backups
```

`--vacuum` физически уменьшает файл, но требует отдельного maintenance window:
API и бот должны быть остановлены, а свободного места должно хватать ещё на одну
копию базы. Без `VACUUM` освобождённые страницы безопасно переиспользуются SQLite,
но размер файла на диске сразу не уменьшается.
