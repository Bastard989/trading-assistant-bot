...
def test_source_registry_is_unique_licensed_and_fail_explicit() -> None:
    ...
    assert payload["version"] == "2026-08-14.3"
    ...
    portwatch = next(
        item for item in payload["sources"] if item["code"] == "imf_portwatch"
    )
    assert portwatch["tier"] == "A"
    assert portwatch["production_status"] == "candidate"
    assert "five allowlisted" in portwatch["rate_limit_policy"]
    assert "shipping/logistics" in portwatch["operational_role"]
